...
int aggiungi_aggr(char nome_file[], int this_d, int this_m, int this_y, char this_tipo, int this_tot)
{
    ...
    sprintf(buf, "%d:%d:%d %c %d", this_d, this_m, this_y, this_tipo, this_tot);
    printf("LOG     Record da aggiungere: %d:%d:%d %c %d\n", this_d, this_m, this_y, this_tipo, this_tot);
    ...
    else
    {
        ...
        while (1) // Copio da 0 alla posizione prima a quella in cui devo scrivere
        {
            ...
            if (data1_minore_data2(d, m, y, this_d, this_m, this_y))
            {
